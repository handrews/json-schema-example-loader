...
ObjectDefinition.prototype.build = function(object) {
  ...
  var self = {
    // A map of properties defined by the object
    all_props: {},
    // All required properties
    required_props: [],
    // Anything that isn't required, keys in all_props
    optional_props: [],
    // The definition for additional properties
  ...
  if (_.isPlainObject(object.properties)) {
    var props = this.defineProperties(object.properties);
    // Track properties at this level.
    _.extend(self.all_props, props);

    // Merge this level's properties down into each *Of subschema.
    _.each(self.objects, function(obj) {
      _.extend(obj.all_props, props);
    });
  }

  if (object.additionalProperties !== undefined) {
    if (_.isPlainObject(object.additionalProperties)) {
        self.additional_props = this.defineProperty(object.additionalProperties);
    } else {
        // additionalProperties can be a boolean.
        self.additional_props = object.additionalProperties;
    }
  }
